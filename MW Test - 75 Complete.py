...
def load_csv(path, group_label, min_progress=0):
    ...
    #### Commented Out to Test a 75% response rate
    ...
    if "Progress" in df.columns and min_progress is not None:
        # Treat missing progress as 0% so they get filtered out
        df["Progress"] = pd.to_numeric(df["Progress"], errors="coerce")
        df = df[df["Progress"].fillna(0) >= min_progress]
...
if __name__ == "__main__":
    combined = prepare_analysis_dataset(FYE_path, NFYE_path, min_progress=75)
